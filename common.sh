...
if [[ `grep -c "CONFIG_PACKAGE_luci-app-bypass_INCLUDE_V2ray=y" ${Home}/.config` -eq '1' ]]; then
...
elif [[ `grep -c "CONFIG_PACKAGE_luci-app-ssr-plus_INCLUDE_V2ray=y" ${Home}/.config` -eq '1' ]]; then
	...
	echo -e "\nCONFIG_PACKAGE_luci-app-ssr-plus=y" >> ${Home}/.config
elif [[ `grep -c "CONFIG_PACKAGE_luci-app-passwall_INCLUDE_V2ray=y" ${Home}/.config` -eq '1' ]]; then
	sed -i 's/CONFIG_PACKAGE_luci-app-passwall_INCLUDE_V2ray=y/# CONFIG_PACKAGE_luci-app-passwall_INCLUDE_V2ray is not set/g' ${Home}/.config
